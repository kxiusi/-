...
def main():
    ...
    pre_adj = pickle.load(open('../datasets/' + opt.dataset + '/pre_adj_dense_12.pkl', 'rb'))
    pre_num = pickle.load(open('../datasets/' + opt.dataset + '/pre_adj_dense_12.pkl', 'rb'))
    global_adj = pickle.load(open('../datasets/' + opt.dataset + '/adj_' + str(opt.n_sample_all) + '.pkl', 'rb'))
    global_num = pickle.load(open('../datasets/' + opt.dataset + '/num_' + str(opt.n_sample_all) + '.pkl', 'rb'))

    pre_adj, pre_num = handle_adj(pre_adj, n_node, opt.n_sample_all, pre_num)
    global_adj, global_num = handle_adj(global_adj, n_node, opt.n_sample_all, global_num)
    model = trans_to_cuda(CombineGraph(opt, n_node, global_adj, global_num, pre_adj, pre_num, ))
