...
class CombineGraph(Module):
    def __init__(self, opt, n_node, adj1, num1, adj2=None, num2=None):
    ...
    def compute_scores(self, hidden, mask):
        ...
        scores = self.linear_three(torch.sigmoid(q1 + q2))
        alpha = entmax_bisect(scores, dim=1)
        # result1 = np.array(alpha.data.cpu())
        # np.savetxt('a_entmax.txt', result1.reshape((hidden.shape[0], -1)))
        #
        # alpha = torch.softmax(scores, dim=1)
        # result2 = np.array(alpha.data.cpu())
        # np.savetxt('a_softmax.txt', result2.reshape((hidden.shape[0], -1)))
    ...
    def forward(self, inputs, A1, A2, mask_item, item):
        ...
        vector1 = aggregator(self_vectors=entity_vectors[0],
        ...
        # vector2 = aggregator(self_vectors=entity_vectors[0],
        #                      neighbor_vector=entity_vectors[1].view(shape),
        #                      masks=None,
        #                      batch_size=batch_size,
        #                      neighbor_weight=weight_vectors[1].view(batch_size, -1, self.sample_num),
        #                      extra_vector=session_info)
        ...
        # alpha3 = torch.sigmoid(self.linear2(h_local))
        # alpha4 = torch.sigmoid(self.linear2(h_global))

        # output = alpha3 * h_local + alpha4 * h_global
