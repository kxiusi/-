...
class GlobalAggregator(nn.Module):
    ...
    def get_alpha(self, x=None, expand_size=None, number=None):
        if number == 0:
            alpha_ent = torch.sigmoid(self.alpha_w(x)) + 1
            alpha_ent = self.add_value(alpha_ent).unsqueeze(1)
            alpha_ent = alpha_ent.expand(-1, expand_size, -1)
            return alpha_ent
        if number == 1:
    ...
    def forward(self, self_vectors, neighbor_vector, batch_size, masks, neighbor_weight, extra_vector=None):
        if extra_vector is not None:
            ...
            scores = torch.matmul(alpha, self.w_2).squeeze(-1)

            alpha = entmax_bisect(scores).unsqueeze(-1)
            result1 = np.array(alpha.data.cpu())
            np.savetxt('g_entmax.txt', result1.reshape((batch_size,-1)))

            alpha = torch.softmax(scores, -1).unsqueeze(-1)
            result2 = np.array(alpha.data.cpu())
            np.savetxt('g_softmax.txt', result2.reshape((batch_size,-1)))
            #
            # alpha_ent = self.get_alpha(x=self_vectors[:, -1, :], expand_size=scores.shape[1], number=0)
