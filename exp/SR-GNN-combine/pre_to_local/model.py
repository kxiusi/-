...
class CombineGraph(Module):
    ...
    def forward(self, inputs, A1, A2, mask_item, item):
        ...
        item_sample_i, weight_sample_i = self.sample(item_neighbors[0], self.adj, self.num)

        item_neighbors.append(item_sample_i.view(batch_size, -1))
        weight_neighbors.append(weight_sample_i.view(batch_size, -1))

        # item_sample_i, weight_sample_i = self.sample(item_neighbors[0], self.adj2, self.num2)
        # item_neighbors.append(item_sample_i.view(batch_size, -1))
        # weight_neighbors.append(weight_sample_i.view(batch_size, -1))
      ...
      #                        neighbor_vector=entity_vectors[2].view(shape),
      #                        masks=None,
      #                        batch_size=batch_size,
      ...
      #   # combine
        ...
        #h_global = self.dropout_global(vector1)
        output = h_local + vector1
        return output
