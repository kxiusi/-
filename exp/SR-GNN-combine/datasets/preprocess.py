...
if opt.dataset == 'diginetica':
    ...
    dataset = '/Users/dmrfcoder/Documents/毕设/code/实验/yoochoose-data/yoochoose-clicks.dat'
    ...
    for data in reader:
        ...
        if curdate and not curid == sessid:
            ...
            if opt.dataset == 'yoochoose':
                ...
                date=time.mktime(time.strptime(curdate, '%Y/%m/%d %H:%M'))
            else:
                date = time.mktime(time.strptime(curdate, '%Y-%m-%d'))
            sess_date[curid] = date
        curid = sessid
        if opt.dataset == 'yoochoose' or opt.dataset == 'mooc':
            item = data['item_id']
        else:
...
if opt.dataset == 'mooc':
    ...
    tra_sess = filter(lambda x: int(x[0]) < splitsession, dates)
    tes_sess = filter(lambda x: int(x[0]) > splitsession, dates)
...
if opt.dataset == 'diginetica':
    ...
    if not os.path.exists('mooc'):
        os.makedirs('mooc')
    pickle.dump(tra, open('mooc/train.txt', 'wb'))
    pickle.dump(tes, open('mooc/test.txt', 'wb'))
    pickle.dump(tra_seqs, open('mooc/all_train_seq.txt', 'wb'))
