...
os.environ[
    'THEANORC'] = '.theanorc_gru4rec'  # Only affects the actual settings if theano was not imported before this point (by any module)
import custom_opt
import datatools
import theano
...
class GRU4Rec:
    ...
    def __init__(self, loss='bpr-max', final_act='linear', hidden_act='tanh', layers=[100],
                 ...
                 adapt='adagrad', adapt_params=[], grad_cap=0.0, bpreg=1.0, logq=0.0,
                 sigma=0.0, init_as_normal=False, train_random_order=False, time_sort=False,
                 session_key='SessionId', item_key='ItemId', time_key='Time'):
        self.layers = layers
    ...
    def cross_entropy(self, yhat, M):
        if self.smoothing:
            ...
            return T.cast(T.sum((1.0 - (n_out / (n_out - 1)) * self.smoothing) * (-T.log(gpu_diag(yhat) + 1e-24)) + (
    ...
    def cross_entropy_logits(self, yhat, M):
        if self.smoothing:
            n_out = M + self.n_sample
            return T.cast(T.sum((1.0 - (n_out / (n_out - 1)) * self.smoothing) * gpu_diag(yhat) + (
    ...
    def bpr_max(self, yhat, M):
        ...
        return T.cast(T.sum(-T.log(T.sum(T.nnet.sigmoid(gpu_diag(yhat, keepdims=True) - yhat) * softmax_scores,
    ...
    def init(self, data):
        ...
        datatools.sort_if_needed(data, [self.session_key])
        ...
        if Y is not None:
            ...
            if predict and self.final_act == 'softmax_logit':
                ...
                if not predict and self.logq:
                    y = y - self.logq * T.log(
                        T.concatenate([self.P0[Y[:M]], self.P0[Y[M:]] ** self.sample_alpha], axis=0))
    ...
    def fit(self, data, sample_store=10000000, store_type='gpu'):
        ...
        if self.logq:
            self.P0 = theano.shared(pop[self.itemidmap.index.values].values.astype(theano.config.floatX), name='P0',
                                    borrow=False)
        if self.n_sample:
        ...
        for epoch in range(self.n_epochs):
            ...
            t1 = time.time()
            dt = t1 - t0
            print('Epoch{} --> loss: {:.6f} \t({:.2f}s) \t[{:.2f} mb/s | {:.0f} e/s]'.format(epoch + 1, avgc, dt,
                                                                                             len(c) / dt,
                                                                                             np.sum(cc) / dt))
        if hasattr(self, 'ST'):
            del (self.ST)
