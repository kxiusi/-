...
PATH_TO_ORIGINAL_DATA = '/home/jywang/code/GRU4Rec/mooc_cube_course/'
PATH_TO_PROCESSED_DATA = '/home/jywang/code/GRU4Rec/processed_data/'

data = pd.read_csv(PATH_TO_ORIGINAL_DATA + 'mooc_cube_new.csv', sep=',', header=None, usecols=[0, 1],
...
train.to_csv(PATH_TO_PROCESSED_DATA + 'mooc_cube_train_full_new.txt', sep='\t', index=False)
print('Test set\n\tEvents: {}\n\tSessions: {}\n\tItems: {}'.format(len(test), test.SessionId.nunique(),
                                                                   test.ItemId.nunique()))
test.to_csv(PATH_TO_PROCESSED_DATA + 'mooc_cube_test_new.txt', sep='\t', index=False)
...
# train_tr.to_csv(PATH_TO_PROCESSED_DATA + 'mooc_cube_train_tr.txt', sep='\t', index=False)
# print('Validation set\n\tEvents: {}\n\tSessions: {}\n\tItems: {}'.format(len(valid), valid.SessionId.nunique(),
#                                                                          valid.ItemId.nunique()))
# valid.to_csv(PATH_TO_PROCESSED_DATA + 'mooc_cube_train_valid.txt', sep='\t', index=False)
