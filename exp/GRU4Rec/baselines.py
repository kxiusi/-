...
class ItemKNN:
    ...
    def __init__(self, n_sims=100, lmbd=20, alpha=0.5, session_key='SessionId', item_key='ItemId', time_key=None):
        self.n_sims = n_sims
        self.lmbd = lmbd
        self.alpha = alpha
        self.item_key = item_key
    ...
    def fit(self, data):
        ...
        index_by_sessions = data.sort_values(['SessionIdx']).index.values
        supp = data.groupby('ItemIdx').size()
        item_offsets = np.zeros(n_items + 1, dtype=np.int32)
        item_offsets[1:] = supp.cumsum()
        index_by_items = data.sort_values(['ItemIdx']).index.values
        for i in range(n_items):
            ...
            pd_data = pd.Series(data=iarray[indices], index=itemids[indices])
            self.sims[itemids[i]] = pd_data.to_dict()
    ...
    def predict_next(self, session_id, input_item_id, predict_for_item_ids):
        ...
        sim_series = pd.Series(sim_list, index=sim_list.keys())
        mask = np.in1d(predict_for_item_ids, sim_series.index)
        preds[mask] = sim_series[predict_for_item_ids[mask]]
        return pd.Series(data=preds, index=predict_for_item_ids)
...
class BPR:
    ...
    def fit(self, data):
        ...
        data = pd.merge(data, pd.DataFrame({self.item_key: itemids, 'ItemId': np.arange(self.n_items)}),
                        on=self.item_key, how='inner')
        data = pd.merge(data, pd.DataFrame({self.session_key: sessionids, 'SessionId': np.arange(self.n_sessions)}),
                        on=self.session_key, how='inner')
        self.init(data)
        for it in range(self.n_iterations):
            ...
            for e in np.random.permutation(len(data)):
                ...
                iidx = data.ItemId.values[e]
                iidx2 = data.ItemId.values[np.random.randint(self.n_items)]
