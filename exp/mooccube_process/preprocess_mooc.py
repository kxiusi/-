...
parser.add_argument('--dataset', default='mooc_cube', help='dataset name: diginetica/yoochoose/sample')
opt = parser.parse_args()

dataset = '%s.csv' % (opt.dataset)
datadict = '%s_dic.csv' % (opt.dataset)
# dataset = 'mooc_cube_video.csv'
print("-- Starting @ %ss" % datetime.datetime.now())
with open(dataset, "r") as f:
    reader = csv.DictReader(f, fieldnames=['session_id', 'item_id'], delimiter=',')
    sess_clicks = {}
    sess_date = {}
    ctr = 0
    curid = -1
...
def obtian_tra():
    ...
    for s, _ in tra_sess:
        ...
        train_ids += [s]
        train_seqs += [outseq]
    print("nodes:", item_ctr)  # 43098, 37484
...
if not os.path.exists('mooc_cube_new'):
    os.makedirs('mooc_cube_new')
pickle.dump(tra, open('mooc_cube_new/train.txt', 'wb'))
pickle.dump(tes, open('mooc_cube_new/test.txt', 'wb'))
pickle.dump(tra_seqs, open('mooc_cube_new/all_train_seq.txt', 'wb'))
